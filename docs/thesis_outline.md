...
本文实验分为仿真实验和真实环境实验两部分。正式仿真对比方法设置为四种，其中前三种用于验证末端风险、连杆级风险和风险约束 SAC 的作用，第四种用于检验风险自适应平滑是否带来额外收益。三-seed 正式结果显示第四种方法未获得稳定收益，因此本文关于整臂避障有效性的核心结论以方法三 `LDRC-SAC-LinkDynamicRisk-FixedSmooth` 的仿真消融结果为主；真实环境实验只部署该固定平滑方法进行低速验证，不对弱基线进行实机碰撞性对比。
...
| LDRC-SAC-LinkDynamicRisk-AdaptiveSmooth | 连杆级动态风险 | 约束 SAC | 风险自适应平滑 | 自适应平滑消融项 |
...
| 自适应平滑是否带来增益 | 方法四 vs 方法三 | 检验自适应平滑是否改善响应性与平滑性折中 |
| 自适应平滑的适用边界 | 方法四 vs 方法三 | 若无稳定收益，将其作为失败消融和后续改进方向 |
...
验证核心方法在无障碍物或低风险条件下能够完成基本目标到达任务，避免出现“只会避障、不会到达”的问题。
...
- `LDRC-SAC-LinkDynamicRisk-FixedSmooth` 能够在目标到达与碰撞控制之间取得较好的折中；安全距离违反率和 jerk 仍须与保守 baseline 一并报告。
...
由于方法三与方法二仅在安全处理方式上不同，且均采用固定低通滤波，该实验可用于支撑“风险约束 SAC 相比固定风险惩罚 SAC 更有利于目标到达与碰撞控制折中”的结论。方法四只作为补充消融；当前三-seed 结果未证明其带来稳定收益。
...
验证仿真训练得到的核心固定平滑方法在真实机械臂平台上的基本可执行性，以及在低速动态障碍物接近非末端连杆时的避障响应能力。
...
| 9 | 真实环境实验只部署 `LDRC-SAC-LinkDynamicRisk-FixedSmooth` |
