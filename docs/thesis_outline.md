...
>
> 新论文暂定题目为“面向感知不确定性动态障碍物的机械臂连杆级预测风险约束控制与协同安全强化学习”。其研究问题、方法、理论边界、实验矩阵和章节结构请以 [research_direction.md](research_direction.md) 为准。阶段一的连杆胶囊建模、动态风险、SAC 管线和失败发现仍是新主题的基础；固定惩罚/约束 SAC 排序和自适应平滑结论仅作为预研究基线，不得与新方法结论混用。
...
对应的可直接插入图件、附录表和重建命令见 `docs/paper_materials.md`。
