...
- 初版“QP 直接修正已生成子步轨迹”在 random_crossing seed 101/1004 100 episode 中将 collision 从 4% 降到 0%，但 physics RMS jerk 从约 110 升到约 280。
...
- random_crossing seed 101/1004 正式 recheck 已完成：smooth-QP 相比 baseline 将 collision 从 4% 降到 0%，safety violation rate 从 1.3865% 降到 0.2015%，physics RMS jerk 基本不变，109.61 -> 109.77。
- 下一轮扩展到 held-out eval seeds 和非末端压力场景，检查 smooth-QP 安全收益是否稳定。
...
- 成功率下降在可解释范围内。
- 介入不是长期保持或频繁停止。
- physics RMS jerk 不应明显高于同 checkpoint baseline；direct-QP 的 jerk 尖峰结果只作为失败诊断。
...
目标：在接触真实机器人前，使用 P6 最终确定的 `Proposed` 配置和 checkpoint，确认模型、观测、安全 QP、降级逻辑和最终指令链路一致。

输入：

```text
P6 最终选定的 Proposed 配置
P6 独立验证选出的 checkpoint
scripts/deployment_preflight.py（需先扩展为支持 Proposed）
outputs/deployment_preflight/proposed_offline_report.json
```

候选约束：

- 旧 `link_fixed_penalty1` EMA checkpoint 只可用于历史链路诊断，不是当前最终实机部署候选。
