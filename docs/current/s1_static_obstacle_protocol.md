...
> 更新时间：2026-08-24。状态：原 S1 final 已冻结；recovery-gated nominal 已完成并通过 gate；t005 goal-velocity temporal-consistency 已成为当前最佳静态 nominal 候选；latency、obstacle-aware nullspace 和 shell+t005 后续分支均拒绝；Residual 训练暂缓。本文只规定 S1 场景和执行细节；动作语义、状态机、安全出口、通用指标与版本规则继承[分层控制通用实验协议](hierarchical_control_protocol.md)。

详细实验谱系、每个失败方向的配置/manifest/output、paired gate、根因和停止理由见 [S1 静态障碍物实验谱系、失败方向与根因登记](s1_static_experiment_registry.md)。本文的命令和实验顺序必须与该登记册一致，不能脱离父版本单独重跑。
...
- （历史记录）诊断版本为独立的 `configs/experiments/hierarchical/s1_static_revised_tracker_v3.yaml`，只在 revised v2 tracker 上增加 filter-aware terminal servo：TCP 速度阻尼、近目标增益 taper、上一周期过滤干预比例反馈；不改变 IK/planner、任务定义或 safety 约束。v3 输出不得写入 v2 目录。
- v3 hard-case 为 `3/23=13.04%`，v4（有限 terminal-stall replan）仍为 `3/23=13.04%`；v4 增加规划开销但没有新增成功，因此 v4 不进入 200-reset final。（历史记录）随后固定 IK/planner，完成了 filter 前后 task-space 速度与误差单调性诊断。
- terminal diagnostics v2 已完成：`nominal_toward_goal_rate=1.0`、`nominal_mean_toward_goal_mps=0.430`，但 `recovery_command_rate=0.4766`，整体 `executed_toward_goal=0.0554 m/s`，说明主要瓶颈是主动 recovery 接管而非 terminal DLS 方向。（历史记录）随后已完成两个 hard-case 归因对照：`s1_static_revised_filter_only_diag.yaml`（关闭主动 recovery，严格 filter 保留）与 `s1_static_revised_recovery_late_diag.yaml`（recovery 延后到边界/短 TTC）。
- hard-case 配对显示 filter-only 为 `8/23`，较 v2 `3/23` 新增 5 个成功且无回归；late-recovery 为 `5/23`。正式候选配置 `s1_static_revised_recovery_gated.yaml` 只允许 `AVOID_HOLD` 使用 recovery，`SERVO` 禁止 recovery 覆盖 nominal DLS；该配置随后已完成同一 `v1_final.json` 的 200-reset nominal gate。Residual 训练仍暂缓。
...
- t005 temporal-consistency nominal 已完成：配置 `configs/experiments/hierarchical/s1_static_revised_recovery_gated_goal_velocity_temporal_consistency_t005_nominal_v1.yaml`，full success `189/200=94.5%`、plan-conditioned `189/189=100%`、IK/plan found `189/200=94.5%`、timeout `0`、collision `0`；相对 recovery-gated nominal paired 为 `187→189`，无 success→failure，暂列当前最佳静态候选。
- t005 latency 变体 hard-case 已完成但拒绝：成功率仍 `12/23=52.17%`，p95 `24.57 ms`、p99 `267.78 ms`、max `1.577 s`，没有改善尾延迟；不运行其 200-reset nominal，也不替换 t005。
- obstacle-aware nullspace IK smoke（`9006`）未通过：局部 clearance-gradient 搜索仍为 `IK_NOT_FOUND`；2000 次多分支 rest-pose 扫描最佳 clearance 约 `0.111m`，低于严格 `0.12m`，不扩展到 23-case。
- `target-shell + t005` 的 `9006` smoke 虽找到路径，但最终为 `FILTER_STOP_TIMEOUT`、误差约 `0.093m`，没有新增成功；该组合不扩展、不进入 nominal。
- 本轮失败根因归因：t005 hard-case 的 `11/23` 失败全为 `IK_NOT_FOUND`；拒绝原因集中在 `goal_error`、`obstacle_clearance`、`obstacle_contact`，而 `solver_error`、`joint_limit`、`self_collision` 均为 `0`。因此不能通过继续增加 IK 调用次数或训练 Residual 解决。
- `9006` 的 raw IK 目标误差可达，但 exact-target 构型 clearance 约 `0.1056m`，低于严格 `d_safe=0.12m`；局部 nullspace 搜索和 2000 次多分支 rest-pose 扫描最佳约 `0.111m`，仍无安全构型证据。shell 能找到路径但在终端阶段 `FILTER_STOP_TIMEOUT`，说明“存在路径”与“12 秒内可完成”是两个独立可行性条件。
...
- goal-region 终端后续分支已冻结：`384` samples、完整 `0.055m` success ball 的 targeted `7` reset 为 `0/7`；`9006/9110` 虽找到合法终端，仍为 `FILTER_STOP_TIMEOUT`。goal-error-first selector、goal-region terminal t005 goal-velocity priority 及二者组合均为 `0/2`，无碰撞、无 paired 新增成功。因此不再扩展该分支到 23-case 或 nominal。
- 历史阶段曾将 nominal 基座冻结回 recovery-gated，并先审计 `9093`、`9143` 两个 `FILTER_STOP_TIMEOUT`；该顺序已由后续 t005 结果更新。当前 nominal 候选为 t005，不训练 Residual。
...
- t005 的验证顺序已完成：`9027` smoke、`9027/9093/9143` triplet、23-reset hard-case 与 200-reset nominal 均通过 success/collision/safe-stop gate；保守近邻阈值为 `0.005 m/s`。仅跳过零请求 LP 的 latency 变体未改善 p99/max，因此不采用。（历史记录）当时剩余失败主类为 IK，随后已完成 obstacle-aware goal-region IK feasibility 审计与后续冻结；不放宽 `d_safe`、success tolerance 或 horizon。

### 5.5 本轮失败归因与决策边界

本轮实验必须区分三类问题：

1. **filter 目标函数问题**：`9093/9143` 的严格可行目标速度为正，而旧投影执行目标速度偏低；HiGHS LP 证明可在同一 hard constraints 内提高目标推进。该问题由 t005 的目标速度优先与时间一致性处理，不能用放宽安全约束解决。
2. **IK/障碍几何问题**：t005 的 11 个 hard-case 失败全部发生在初始 IK 阶段。审计显示 17 个 reset raw goal reachable，其中 5 个最终被 clearance/contact 拒绝；`9006` 的局部和多分支搜索均未找到 `d_min≥0.12m` 的 exact-target 构型。这是当前主要结构性瓶颈。
3. **终端时限问题**：`9006` 的 target-shell 分支能建立路径，却以 `FILTER_STOP_TIMEOUT` 结束、误差约 `0.093m`；说明 shell 只改善了 IK/plan 入口，未证明终端 strict filter/recovery 下具备足够的时限内推进能力。

最终决策：t005 保持当前最佳静态 nominal 候选（`189/200` full、`189/189` plan-conditioned、collision `0`）。nullspace、shell+t005 和 latency 变体均不进入 23-case 或 nominal final；不训练 Residual，不放宽 `d_safe=0.12m`、success tolerance `0.055m` 或 12 秒 horizon。后续若继续，必须做独立 obstacle-aware goal-region IK 离散分支搜索，并以 paired success、IK found、最终误差、collision 和 filter latency 联合验收。
...
以下是本轮已经执行过的历史顺序，用于复现实验谱系；不是当前待运行队列。当前是否允许新实验，以实验登记册和本协议第 7 节为准。
...
7. `ik_predictive_v2`、target-shell v1 及全部 goal-region terminal 后续分支均无 paired 增益；不运行其 nominal final。历史上曾保留 recovery-gated 作为 nominal 基座并分析 `9093/9143`，该分析已完成；当前最佳静态候选已更新为 t005，后续不得回到旧 timeout 方向重复运行。
...
9027 时间一致性 smoke（历史命令；已执行，不得重复运行）：
...
该 smoke 已成功，随后三例 paired 也已成功。下面命令仅保留作复现记录；再次运行前必须新建版本号、output 目录和 registry 条目，不得覆盖现有结果：
...
- S1-R 已完成至少 3 个训练 seed，并与当前冻结的 S1-N/t005 完成成对消融；本轮该条件尚未满足，Residual 训练暂缓；
...
本协议中的运行命令包含历史实验的复现命令，不代表这些方向仍获准运行。启动任何新命令前，必须先查 [S1 实验登记册](s1_static_experiment_registry.md)，并完成以下检查：

1. 明确父 YAML、父版本的失败主类和同 seed old CSV；
2. 新建唯一的配置、manifest、output 目录和 architecture version；
3. 用 `load_config` 或等价方式打印 effective config，确认 `d_safe=0.12m`、success tolerance `0.055m`、12 s horizon、状态机和唯一 safety-filter 出口未漂移；
4. 先做最小 targeted smoke；只有出现机制证据和至少一个 `old_failure→new_success`，才允许完整 23-reset paired；
5. 只有完整 hard-case paired 满足 `collision=0`、`old_success→new_failure=0`、`old_failure→new_success>=1` 且 plan-conditioned success 达标，才允许 200-reset nominal；
6. 任何未通过 gate 的方向必须登记为 stopped/rejected，禁止以“再跑一次 23-case”替代根因分析。

当前明确禁止重复的方向：更多 goal-region samples、clearance/predictive/goal-error selector、terminal retain/hold/slow/track/waypoint、goal-region terminal priority 及其组合、继续增加普通 IK 调用次数、只跳过零请求 LP 的 latency 变体。若新假设仍属于上述机制，必须停止而不是创建新 YAML。
