...
点雅可比实现已将 PyBullet 内过滤器计算保持在当前 50 ms 仿真周期内；当前 100k 开发轮次已完成独立 checkpoint validation，但 B1--B5 尚未形成稳定综合候选。过滤器现支持循环投影、Dykstra、主动集回退和可选 OSQP QP；OSQP 依赖已安装，已有诊断平均求解约 6--9 ms。此前最终投影失败路径丢失 OSQP 状态，已修复，必须先用同一 checkpoint 和 eval seed 重跑状态化 trace，再判断是真实不可行还是求解精度问题。该测量不包含感知、通信和控制器延迟。
...
| P2 | 仿真安全过滤器 | **开发验证完成，求解器诊断进行中**：约束投影、Dykstra/主动集回退、可选 OSQP QP、端到端命令出口、无效/过期状态注入和 smoke test 通过；仍未构成端到端实时性或安全效果结论 |
| P3 | 协同训练与消融 | **100k 开发轮次完成，未冻结**：B1--B5 单开发 seed 未显示稳定综合收益；OSQP 失败状态透传修复后需重跑，再决定是否诊断参数或停止扩展 |
