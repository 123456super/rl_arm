...
点雅可比实现已将 PyBullet 内过滤器 P95 求解时间降至约 5 ms，满足当前 50 ms 仿真控制周期；该测量不包含感知、通信和控制器延迟。下一轮开发应保持 B1--B5 因子和参数不变，将共同训练预算扩展至 100k，并使用独立 checkpoint validation；仅在完整开发预算仍显示合理任务--安全折中时才冻结设计并进入 P4。
...
| P2 | 仿真安全过滤器 | **已完成开发验证**：约束投影、回退、端到端命令出口、无效/过期状态注入和 PyBullet smoke test 通过；点雅可比版本在 PyBullet 内满足 50 ms 周期，但仍未构成端到端实时性或安全效果结论 |
| P3 | 协同训练与消融 | **短训练已完成，未冻结**：B1--B5 单开发 seed 检查未显示稳定综合收益；先扩展共同开发预算至 100k 并进行 checkpoint validation |
