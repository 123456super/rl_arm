...
**阶段一已闭环；当前进入“预测风险 + 安全过滤器 + 协同安全强化学习”的研究重构阶段。**

主比较的模型训练、checkpoint selection、eval-seed held-out 评估和三-seed汇总均已完成；它们被冻结为阶段一基线，当前不需要继续训练或重跑。新研究的实施基准见 [research_direction.md](research_direction.md)：先完成不确定性预测风险和仿真安全过滤器，再开展协同训练、独立复核和低速真机验证。
...
1. 实现并单元验证不确定性连杆预测风险：误差裕度、时延、状态过期和无效检测必须有确定性行为。
2. 实现仿真在线安全过滤器：策略输出必须经由过滤器；不可行、过期或无效状态必须安全停止。
3. 以阶段一方法为基线，完成风险表示与过滤器的因子化消融；在开发阶段冻结设计后，使用新 train/eval seeds 复核。
4. 现场验证从风险检出、干预提前量和失效安全开始，而不是比较真实碰撞率。
