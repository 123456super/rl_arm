...
> **新主题关系**：本文件只覆盖阶段一策略的离线预检，不是“预测风险 + 安全过滤器”新系统的部署批准。新系统必须新增预测风险误差裕度、过滤器可行性、过滤器延迟、不可行回退、感知失效和实机控制器接口的检查；设计边界见 [research_direction.md](../../design/research_direction.md)。
