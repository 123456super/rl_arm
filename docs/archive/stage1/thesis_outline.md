# 面向单动态障碍物的机械臂连杆级动态风险 SAC 避障研究

> **本文件状态（2026-07-31）**：以下内容完整保留阶段一的原始论文大纲、技术细节和实验设计，便于复现旧基线与追溯设计决策。它不再是本分支拟写论文的现行大纲；P3 预测风险/安全过滤分支仍处于仿真失效分析，未进入 P4/OOD 或真机验证。
>
> **归档入口（2026-08-03）**：当前研究状态和结果从 [docs/README.md](../../README.md) 进入；本文件不再更新任何“当前状态”。
>
> 新论文暂定题目为“面向感知不确定性动态障碍物的机械臂连杆级预测风险约束控制与协同安全强化学习”。其研究问题、方法、理论边界、实验矩阵和章节结构请以 [research_direction.md](../../design/research_direction.md) 为准。阶段一的连杆胶囊建模、动态风险、SAC 管线和失败发现仍是新主题的基础；固定惩罚/约束 SAC 排序和自适应平滑结论仅作为预研究基线，不得与新方法结论混用。

## 阶段一目录

- [面向单动态障碍物的机械臂连杆级动态风险 SAC 避障研究](#面向单动态障碍物的机械臂连杆级动态风险-sac-避障研究)
  - [目录](#目录)
  - [一、研究背景与意义](#一研究背景与意义)
  - [二、研究目标](#二研究目标)
  - [三、拟创新点](#三拟创新点)
...
本文同时考察固定风险惩罚 SAC 与风险约束 SAC 两种安全处理方式，并在相同的连杆级动态风险输入和固定平滑条件下进行比较。固定惩罚权重通过小规模筛选标定；约束 SAC 使用风险代价和拉格朗日乘子调节训练阶段的安全强度。

同时，强化学习策略直接输出关节速度时容易出现动作抖动，而固定低通滤波又可能削弱高风险阶段的避障响应。为此，本文将动作平滑限定为执行层补充模块，用于改善部署阶段的平滑性与响应性。

本文主线压缩为“连杆级动态风险建模 - 安全处理方式比较 - 固定平滑执行与验证”。本文以固定基座 UR5 机械臂在单动态球形障碍物环境下的静态目标位置到达任务为研究对象，将机械臂主要连杆纳入风险评估范围，并将连杆级动态风险用于固定惩罚和约束两类 SAC 的训练。本文不宣称提供严格形式化安全保证，而是通过仿真实验和低速真实环境验证任务完成、碰撞控制与风险响应。

本文围绕一个核心问题展开：**连杆级动态风险建模与不同安全处理方式如何影响机械臂在动态障碍物靠近非末端连杆时的任务完成和碰撞控制**。最终 held-out 结果表明，经标定的固定惩罚方法综合表现更好；约束 SAC 仅在部分区域保留略低碰撞率。实验结论据此审慎归因，不将完整系统性能提升简单归因于约束机制或自适应平滑模块。

## 二、研究目标

本文面向固定基座 UR5 机械臂在单动态障碍物环境下的静态目标位置到达与整臂避障控制问题，主线概括为：连杆级动态风险建模、固定惩罚与约束 SAC 比较、固定平滑执行与验证。

1. 构建 UR5 整臂几何模型与连杆级动态风险模型，统一描述距离、方向、接近速度和径向近似 TTC，并形成可用于学习与评价的风险代价。
2. 将静态目标到达与动态避障分别纳入固定风险惩罚 SAC 和 CMDP 约束 SAC，在相同连杆级风险输入下比较任务、安全与平滑性权衡。
3. 搭建 PyBullet 仿真与 UR5 低速真实验证平台，通过碰撞率、最小距离、安全距离违反率、目标到达误差、关节加速度和 jerk 等指标完成 held-out 对比与部署验证。
...
- 将机械臂静态目标到达与动态避障问题分别建模为固定惩罚与安全代价约束的强化学习问题，分析固定风险权重和拉格朗日约束机制带来的差异。
- 将风险自适应动作平滑作为失败消融保留，分析其 actor 退化与 jerk 增大的原因，不将其纳入核心方法或部署候选。
- 搭建 PyBullet 仿真实验平台和基于 RGB-D 相机的低速真实环境验证系统，通过碰撞率、最小距离、安全距离违反率、目标到达误差、关节加速度和 jerk 等指标验证连杆级风险与经标定固定惩罚配置在本文设定场景下的表现。

## 三、拟创新点

本文贡献与实验结论分开表述，避免将未经支持的机制优势作为创新结论。

1. **核心研究内容：连杆级动态风险表征。** 面向单动态障碍物靠近非末端连杆的场景，本文将距离、方向、接近速度和径向近似 TTC 组成连杆级风险状态与风险代价，使策略保留风险发生部位的信息。
2. **比较研究内容：两类安全处理方式。** 本文在相同连杆级风险输入和固定平滑条件下比较固定风险惩罚 SAC 与约束 SAC。最终 held-out 结果不支持约束 SAC 对经标定固定惩罚 SAC 的整体优势，因此该部分定位为方法比较与局限分析。
3. **失败消融：风险自适应动作平滑。** 该机制未获得稳定收益，且提高 jerk；仅作为后续改进方向，不作为核心创新或部署模块。
...
- **方法边界**：本文重点研究连杆级动态风险表征与代价建模，并比较固定风险惩罚 SAC 和风险约束 SAC 两种安全处理方式。最终主方法为连杆级固定风险惩罚 SAC（`w_R=1.0`、固定平滑）；风险自适应动作平滑仅作为失败消融保留。本文不提供严格安全性理论证明，不研究控制屏障函数、模型预测控制等形式化安全控制方法。
...
本文当前的仿真实现使用 PyBullet 点雅可比，并固定当前最优投影参数构造连杆与 TCP 的约束雅可比，以避免控制周期内扰动并保存/恢复仿真状态。有限差分可作为离线验证基准：
...
与传统固定风险奖励惩罚不同，约束 SAC 通过拉格朗日乘子 lambda 在训练阶段自适应调节风险代价对策略更新的影响。该机制并不消除人工参数设定，而是将安全调节方式从奖励函数中的固定惩罚权重转移到约束阈值 `C_safe` 与 lambda 的自适应更新上；最终 held-out 结果未证明这种转移带来整体性能优势。
...
约束 SAC 的机制作用在于将任务奖励和安全代价分开建模，并在训练过程中根据风险代价水平调节安全约束强度。该机制可作为比较对象，但本文最终 held-out 结果仅显示其在部分区域具有略低碰撞率，不能据此主张其整体优于经标定的固定风险惩罚 SAC。
...
本文实验分为仿真实验和真实环境实验两部分。最终主比较使用 `ee_fixed`、`link_fixed_penalty1` 和 `ldrc_fixed` 三种方法，以及 3 个 train seeds、3 个 held-out eval seeds 和 5 个场景。结果显示 `link_fixed_penalty1`（连杆级固定风险惩罚 SAC，`w_R=1.0`）获得最高任务完成表现，并在多数安全和运动指标上优于 `ldrc_fixed`；后者仅在部分场景保留略低碰撞率。原四方法矩阵和自适应平滑仅作为历史消融，不参与正文主表或实机部署候选。

### 7.1 仿真实验平台

仿真实验采用 PyBullet 搭建 UR5 机械臂环境，使用固定风险惩罚 SAC 与风险约束 SAC 作为连续控制算法。机械臂采用关节速度控制，任务目标为静态目标点到达，障碍物为单个动态球体。
...
| 控制方式 | 关节速度控制 |
| 基础强化学习算法 | SAC |
| 主比较算法 | 连杆级固定风险惩罚 SAC（`w_R=1.0`）与连杆级动态风险约束 SAC |
| 任务类型 | 静态目标点到达 |
| 障碍物 | 单个动态球体 |
| 避障对象 | UR5 主要连杆 |
| 连杆模型 | 胶囊体 |
...
真实环境实验不进行在线强化学习训练，仅部署 `link_fixed_penalty1` 的仿真 checkpoint，用于验证实际执行可行性和基本避障效果。
...
2. 方法二：SAC-LinkDynamicRisk-FixedPenalty1-FixedSmooth（`w_R=1.0`）
3. 方法三：LDRC-SAC-LinkDynamicRisk-FixedSmooth

三种方法均以“风险信息来源 + 安全处理方式 + 平滑方式”三段式命名：

| 方法名称 | 风险信息 | 安全处理方式 | 平滑方式 | 作用 |
| --- | --- | --- | --- | --- |
| SAC-EndEffectorRisk-FixedPenalty-FixedSmooth | 末端执行器风险 | 固定惩罚 SAC | 固定低通滤波 | 末端风险基线 |
| SAC-LinkDynamicRisk-FixedPenalty1-FixedSmooth | 连杆级动态风险 | 固定惩罚 SAC，`w_R=1.0` | 固定低通滤波 | 经标定的连杆级固定惩罚方法 |
...
| 需要验证的问题 | 对比方式 | 可支持的结论 |
| --- | --- | --- |
| 连杆级风险加经标定固定惩罚是否优于末端风险 | 方法二 vs 方法一 | `link_fixed_penalty1` 在 held-out 五场景中具有更高成功率和更低最终误差 |
| 约束 SAC 与经标定固定惩罚的取舍 | 方法三 vs 方法二 | `ldrc_fixed` 仅在部分区域碰撞率略低，未显示整体增益 |
| 固定风险权重的影响 | 参数筛选 + 方法二 | `w_R` 是强敏感超参数，主表须报告 `w_R=1.0` 的筛选流程和局限 |

原 `ldrc_adaptive` 仅作为失败消融：其自适应 actor 在 seed 303 退化，执行器也提高 jerk，故不作为完整方法展示、核心贡献或真实部署候选。

固定风险惩罚 SAC 的公平性设置如下：

- 方法二的 `w_R=1.0` 由 train seeds 101、202 的小规模筛选提出，并以 3 个 train seeds 重训。
- 最终主表使用 held-out eval seeds 1004--1006，避免复用筛选阶段的 eval seeds；但训练 seeds 部分复用，不能将该流程表述为完全独立的超参数验证。
- 论文中报告该选择流程和局限，避免弱基线或配置选择偏差导致不公平对比。
...
验证最终部署候选方法在无障碍物或低风险条件下能够完成基本目标到达任务，避免出现“只会避障、不会到达”的问题。

测试方法：

`SAC-LinkDynamicRisk-FixedPenalty1-FixedSmooth`（`link_fixed_penalty1`，`w_R=1.0`）
...
- SAC-EndEffectorRisk-FixedPenalty-FixedSmooth（`ee_fixed`）
- SAC-LinkDynamicRisk-FixedPenalty1-FixedSmooth（`link_fixed_penalty1`，`w_R=1.0`）
- LDRC-SAC-LinkDynamicRisk-FixedSmooth（`ldrc_fixed`）
...
该实验使用 train seeds 101、202、303 和 held-out eval seeds 1004、1005、1006；每种方法在每个场景评估 900 episodes。正文仅报告 Random Crossing 主表和四个定向压力场景表，统计单位为 train seed（`mean +/- std`, `n=3`）。

该实验支持以下结论：

- `link_fixed_penalty1` 在五个场景均取得最高三-seed平均成功率，并明显降低最终位置误差。
- `link_fixed_penalty1` 在最终误差、最小距离、安全距离违反率和 jerk 上优于 `ldrc_fixed`；后者仅在部分场景保留略低碰撞率。
- 不支持将约束 SAC 表述为相对经标定固定惩罚 SAC 的整体最优方法，也不支持任何方法在全部安全指标上严格占优。

#### 正文结果表

表 1 和表 2 使用 `outputs/rechecks/heldout_1004_1006/final_3methods/` 下的汇总数据。每个单元格先合并同一 train seed 下的 300 episodes，再报告 3 个 train seeds 的 `mean +/- sample std`（`n=3`）；每个场景-方法共 900 held-out episodes。

**Table 1：Random Crossing held-out 主对比**

| 方法 | success rate | collision rate | non-end-link collision | final position error | min distance | safety violation rate | RMS jerk |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `ee_fixed` | 17.00% +/- 2.00% | 13.44% +/- 5.19% | 13.11% +/- 5.17% | 0.369 +/- 0.017 m | 0.177 +/- 0.012 m | 2.38% +/- 1.44% | 25.0 +/- 2.0 |
| `link_fixed_penalty1` | 64.22% +/- 12.54% | 6.89% +/- 2.22% | 6.89% +/- 2.22% | 0.125 +/- 0.029 m | 0.193 +/- 0.020 m | 1.27% +/- 0.92% | 32.3 +/- 2.7 |
| `ldrc_fixed` | 56.33% +/- 18.02% | 5.56% +/- 2.91% | 5.56% +/- 2.91% | 0.172 +/- 0.054 m | 0.167 +/- 0.008 m | 3.12% +/- 1.27% | 32.7 +/- 0.2 |

**Table 2：非末端区域 held-out 压力测试**

| 区域 | 方法 | success rate | collision rate | final position error | RMS jerk |
| --- | --- | ---: | ---: | ---: | ---: |
| upper arm | `ee_fixed` | 13.33% +/- 0.58% | 3.00% +/- 3.61% | 0.414 +/- 0.034 m | 25.6 +/- 1.4 |
| upper arm | `link_fixed_penalty1` | 56.67% +/- 17.15% | 11.67% +/- 7.51% | 0.154 +/- 0.063 m | 34.8 +/- 3.9 |
| upper arm | `ldrc_fixed` | 54.22% +/- 19.31% | 9.33% +/- 0.58% | 0.180 +/- 0.083 m | 35.5 +/- 1.5 |
| elbow | `ee_fixed` | 16.67% +/- 1.15% | 5.67% +/- 3.21% | 0.381 +/- 0.008 m | 25.2 +/- 1.7 |
| elbow | `link_fixed_penalty1` | 63.78% +/- 16.45% | 8.67% +/- 5.03% | 0.135 +/- 0.049 m | 33.0 +/- 3.0 |
| elbow | `ldrc_fixed` | 56.56% +/- 24.06% | 7.33% +/- 2.31% | 0.169 +/- 0.079 m | 35.3 +/- 1.7 |
| forearm | `ee_fixed` | 19.44% +/- 5.50% | 5.33% +/- 3.21% | 0.390 +/- 0.029 m | 23.9 +/- 1.5 |
| forearm | `link_fixed_penalty1` | 72.78% +/- 16.18% | 3.00% +/- 2.65% | 0.109 +/- 0.037 m | 33.1 +/- 3.9 |
| forearm | `ldrc_fixed` | 67.00% +/- 18.21% | 4.33% +/- 1.15% | 0.131 +/- 0.052 m | 34.8 +/- 0.7 |
| wrist | `ee_fixed` | 15.78% +/- 3.24% | 11.00% +/- 3.46% | 0.429 +/- 0.038 m | 23.8 +/- 2.1 |
| wrist | `link_fixed_penalty1` | 76.11% +/- 14.73% | 2.33% +/- 2.52% | 0.099 +/- 0.031 m | 32.0 +/- 2.5 |
| wrist | `ldrc_fixed` | 62.33% +/- 25.44% | 3.00% +/- 1.00% | 0.147 +/- 0.071 m | 33.8 +/- 1.3 |

对应的可直接插入图件、附录表和重建命令见 `docs/archive/stage1/paper_materials.md`。

### 7.7 仿真实验三：风险约束 SAC 机制分析

实验目的：

分析风险约束机制的训练行为和它相对经标定固定惩罚的局部取舍，不将该分析作为约束 SAC 整体优于固定惩罚的证据。

重点比较：

| 类型 | 方法 |
| --- | --- |
| 主要对比 | SAC-LinkDynamicRisk-FixedPenalty1-FixedSmooth（`w_R=1.0`） |
...
- 风险约束 SAC 是否在 upper arm、elbow 等高风险区域呈现较低碰撞率。
- 固定惩罚方法是否在 Success Rate、Final Position Error、Minimum Distance、Safety Violation Rate 和 jerk 上保持优势。
...
由于方法三与方法二仅在安全处理方式上不同，且均采用固定低通滤波，该实验用于说明取舍：`ldrc_fixed` 的宏平均碰撞率略低（5.91% vs 6.51%），但 `link_fixed_penalty1` 在成功率、最终误差、最小距离、安全距离违反率和 jerk 上更好。固定惩罚权重是强敏感超参数；`w_R=1.0` 的筛选与重训部分复用了 train seeds，因此该结果仍需新 train seeds 复核。

### 7.8 仿真实验四：风险自适应平滑机制分析

该部分仅作为历史失败消融，基于原四方法矩阵的方法三和方法四进行对比，并结合典型 episode 曲线进行机制性分析。
...
该部分用于检验风险自适应平滑机制是否带来执行收益。当前三-seed 正式结果和反事实诊断均未证明其改善平滑性或安全性：该执行器提高了 jerk，且 adaptive actor 在 seed 303 上出现策略退化。因此本节应作为失败消融与后续改进方向，不作为本文主要贡献。

### 7.9 真实环境低速整臂避障验证

实验目的：

验证仿真训练得到的 `link_fixed_penalty1` 固定平滑方法在真实机械臂平台上的基本可执行性，以及在低速动态障碍物接近非末端连杆时的避障响应能力。
...
8. 策略输出关节速度，经固定平滑、速度限幅和工作空间检查后发送给机械臂控制接口。
...
| 结果展示 | 典型实验轨迹；`d_min`、`Risk_global` 的变化曲线；机械臂对非末端连杆风险的避让过程 |
...
| 参数 | 分析目的 | 建议设置 |
| --- | --- | --- |
| 固定风险惩罚权重 w_R | 分析固定惩罚 SAC 对风险权重的敏感性，并说明最终 `w_R=1.0` 配置的选择依据与局限。 | 选取低、中、高 3 到 5 个候选值。 |
...
本文对参数敏感性结论的表述应保持审慎：风险约束 SAC 并不消除所有人工参数，而是将安全与任务折中的调节方式从固定奖励惩罚权重转移为约束阈值和拉格朗日乘子自适应调节。当前结果不能证明该机制减弱了固定惩罚权重选择对最终性能的影响。
...
失败案例分析用于限定论文结论的适用范围。本文结论应限定为：在单动态球形障碍物、低到中等速度、感知可获得且机械臂速度受限的场景下，连杆级动态风险配合经标定的固定风险惩罚可改善任务完成和多数安全指标；不外推为约束机制的整体优势或形式化安全保证。
...
| 8 | 主比较方法使用 3 个 train seeds；正文表使用 held-out eval seeds 1004--1006，并报告 `mean +/- std`（`n=3`） |
| 9 | 真实环境实验只部署 `link_fixed_penalty1`，且仅做低速可执行性验证 |
...
| 资源不足时优先级 | 已完成三种主比较方法；后续复核优先增加新的 train seeds |
| 仿真测试量 | 每种方法测试 100 个 episode |
| 真实环境测试量 | `link_fixed_penalty1` 低速测试 10 到 20 次 |
| 结果统计 | 报告 mean ± std |
