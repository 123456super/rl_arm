...
| `historical/two_methods_initial/` | 已替代 | 早于 `ee_fixed` 补评估生成的两方法中间汇总，共 9,000 episodes；不得用于论文主表或最终排序。 |
...
`outputs/formal/summary/` 是另一套旧四方法实验，使用不同的 eval seeds 和固定惩罚配置，只能作历史或敏感性分析。
